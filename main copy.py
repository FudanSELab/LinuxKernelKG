...
from search_all_sources import MultiSourceSearcher
...
async def run_pipeline():
    ...
    try:
        with EnhancedNeo4jHandler(**kg_config.neo4j_config) as neo4j_handler:
            ...
            # 随机抽取300个feature并保存
            # 确保抽样数量不超过总数
            sample_file = output_dir / f"sampled_features_final.csv"
            
            # 检查文件是否存在
            if sample_file.exists():
                # 如果文件存在，读取文件内容
                df = pd.read_csv(sample_file, encoding='utf-8')
                logger.info(f"Loaded features from existing file: {sample_file}")
                
                # 从文件中加载特征到 sampled_features
                sampled_features = df.to_dict('records')  # 将 DataFrame 转换为字典列表
            else:
                # 如果文件不存在，进行随机采样
            ...
            # 初始化 MultiSourceSearcher
            searcher = MultiSourceSearcher()
            
            # 处理每个feature
            for i, feature in enumerate(sampled_features[start_index:], start=start_index):
                ...
                # 使用 MultiSourceSearcher 处理提取结果
                search_results = searcher.process_extraction_result(extraction_result)
                logger.info(f"Search results: {search_results}")
                
                # 新增：将 search_results 保存到文件
