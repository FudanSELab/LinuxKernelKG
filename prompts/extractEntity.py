...
class extractEntityPrompt:
    ...
    template = """{{
    "instructions": "你是一个命名实体抽取 (NER) 任务的专家，同时也是 linux 内核的专家。给你一个 linux 内核特性的描述 (feature_description)、对应的若干 commit 的标题和提交信息 (commits)。你的任务是：根据这些信息提取出在 linux 内核中涉及到的和这个特性相关的概念实体 (concepts)。用英语回答。用json格式字符串返回你的回答。你可以参考 examples 字段给出的例子。",
    "examples": [
        "input": {{
