...
class extractTriplePrompt:
    ...
    template = """{{
    "instructions": "你是一个开放关系抽取 (OpenIE) 任务的专家，同时也是 linux 内核的专家。给你一个 linux 内核特性的描述 (feature_description)、对应的若干 commit 的标题和提交信息 (commits)、以及这个特性涉及的一些 linux 内核相关的实体 (entities)。你的任务是：根据这些信息提取出在 linux 内核中涉及到的与这个 feature 相关关系三元组 (triples)，形如（实体1，关系，实体2）。用英语回答。用json格式字符串返回你的回答。你可以参考 examples 字段给出的例子。",
    "examples": [
        "input": {{
            ...
            "entities": ["subpage", "Large folio", "Memory availability", "Anti-fragmentation"]
        }},
        "output": {{
...
"""
    ...
    prompt_template_extract = PromptTemplate(template=template, input_variables=["feature_description", "commits", "entities"])

    def __init__(self, feature_description, commits, entities):
