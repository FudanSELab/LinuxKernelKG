...
class verifyPrompt:
    ...
    template = """{{
    "instructions": "你是一个知识图谱专家，同时也是 linux 内核的专家。给你一个 linux 内核特性的描述 (feature_description)、对应的若干 commit 的标题和提交信息 (commits)，以及从中抽取出的实体 (entities) 和关系三元组 (triples)。你的任务是：验证这些实体和关系三元组是否合理，只保留合理的结果。合理的实体和关系三元组需要满足：1. 在 linux 内核中存在；2. 不是只在给出的提交中出现，而是在整个 linux 内核中存在的实体和成立的关系；3. 实体必须是能够独立存在的概念名词，关系(e1, r, e2)中e1必须是实体，e2可以是实体或者是属性。用英语回答。用json格式字符串返回你的回答，不要输出多余内容。你可以参考 examples 字段给出的例子。",
    "examples": [{{
        "input": {{
        ...
        "output": {{
            ...
            "reason": "The entities and triples are reasonable except for the invalid_entity, invalid_entity1 and invalid_entity2, which are not in the linux kernel. Moreover, the triple 'This wasted memory remains allocated until it is either unmapped or memory reclamation occurs' is not a valid triple as 'until it is either unmapped or memory reclamation occurs' is not an entity."
        }}
    }}],
    "input": {{
